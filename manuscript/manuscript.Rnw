...
\EV are small (40-1000nm) compartments that can be found in
blood, saliva, and urine. \EV originate from the plasma membrane
or multivesicular endosome of many cell types and contain proteins,
lipids, mRNA, miRNA, and many other non-coding RNAs. Because \EV
...
The population of RNAs found in \EV are distinct from the cytosolic
pool, and are likely selectively sorted into \EV~%
...
\EV. Owing to the limited number of tissue-specific \ac{EV} RNA-seq
...
invisible(tissue.specific.genes[,type:="not specific"])
invisible(tissue.specific.genes[,housekeeping:="not housekeeping"])
invisible(tissue.specific.genes[possibly.specific==TRUE,type:="tissue specific"])
invisible(tissue.specific.genes[tracking_id %in% housekeeping.genes.superset[,tracking_id] &
                                 possibly.specific==FALSE,housekeeping:="housekeeping"])
invisible(tissue.specific.genes[tracking_id %in% housekeeping.genes.superset[,tracking_id] &
                          possibly.specific==TRUE,housekeeping:="housekeeping and specific"])

invisible(tissue.specific.genes[,type:=
                          factor(type,
                                 levels=c("not specific","tissue specific",
                                     "housekeeping",
                                     "housekeeping and specific"
                                          ))])
print(ggplot(tissue.specific.genes[gi.max>0],
             aes(y=specificity,x=gi.max,colour=type))+
      ##geom_point()+
      geom_hex(bins=50)+
      ## stat_summary_2d(fun=function(x){length(x)},bins=100)+
      geom_point(aes(colour=housekeeping),data=tissue.specific.genes[housekeeping!="not housekeeping",])+
      scale_x_log10()+
...
@ 
\caption{Shape of tissue specific genes}
...
print(xtable(temp,
             caption="Uterus specific genes",
             label="tab:uterus_specific_genes"),
