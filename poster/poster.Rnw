...
\begin{columns}
  ...
  \block{Introduction}{\acp{EV} are small (40-1000nm) compartments
    ...
    and mRNA markers which are diagnostic of multiple organs and
...
\column{0.25}
  \block{Genes Diagnostic of Uterus}{
...
print(specific.markers.plot("uterus")
      + xlab("Genes Diagnostic of Uterus ")
      + ylab("Tissue"))
@
}
\captionof{figure}{Expression of genes diagnostic of uterus
  ($\tau_g \ge 0.98$) in uterus and all tissues with the second
  highest expression for any of the uterus-diagnostic genes
  ordered by their tissue specificity. The known uterus-diagnostic
  gene PAEP and short RNAs ZNFX1-AS1\_3 (87bp) and AL356310.1 (94bp)
  are highly diagnostic of uterus.}

}

  \block{Genes Diagnostic of Placenta}{
{\centering
...
print(specific.markers.plot("placenta",
                            genes=as.character(tissue.specific.genes[tissue=="placenta" & 
                                                                         gi.max >= 1024
                               ,gene]))
      + xlab("Genes Diagnostic of Placenta")
      + ylab("Tissue"))
@

}
\captionof{figure}{Expression of genes diagnostic of placenta
  ($\tau_g \ge 0.98$) with high expression ($\mathrm{FPKM} \ge 1024$)
  in placenta and all tissues with the second highest expression for
  any placenta-diagnostic gene ordered by their specificity to
  placenta. The previously known placenta-diagnostic PSG genes and
...
\block{Conclusions}{
  ...
  \item Pregnancy-specific glycoproteins (PSG1, 2, 3 et al.) are
    diagnostic of placental tissues, and PAEP is diagnostic of
