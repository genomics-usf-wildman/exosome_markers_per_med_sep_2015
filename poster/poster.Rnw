...
\renewcommand\TP@maketitle{%
   \begin{minipage}{0.8\textwidth}
    ...
    \begin{minipage}{0.2\linewidth}
      ...
      \includegraphics[width=0.8\linewidth,height=6cm,keepaspectratio]{figures/uofi_mark}
    \end{minipage}
}
\makeatother
...
<<load.data,echo=FALSE,results="hide",warning=FALSE,cache.extra=tools::md5sum(paste0("../tissue_specific_expression/",c("tissue_specific_markers","interesting_gene_reads_wide","combined_star_logs","svm_test_results")))>>=
...
%     individual patient. 
  ...
  \block{Methods}{ %
    ...
    ($\approx 820$ samples). For comparison purposes, we also
    generated a \ac{KNN} based predictor using the same training and
    testing sets of genes.
...
\captionof{figure}{Uterus specific genes. $\log_2 \mathrm{FPKM}$ of
  ...
  multiple fetal tissues and MYH11, a major component of smooth
  muscle, all genes are highly specific to uterus.}
...
\captionof{figure}{\ac{ROC} curve of the placenta \ac{SVM}. The
  placenta \ac{SVM} is fairly effective at distinguishing between
  samples of placental origin and non-placental origin.}

  
  % ROC curve for uterus/not uterus

<<uterus.roc.curve,fig.width=8,fig.height=8,echo=FALSE>>=
ut.roc.obj1 <- 
...
legend("bottomright",
       ...
       lwd=2)
@ 
\captionof{figure}{\ac{ROC} curve of the uterus \ac{SVM}. The uterus \ac{SVM} is
  highly effective at distinguishing between samples of uterus origin and
  non-uterus origin}
