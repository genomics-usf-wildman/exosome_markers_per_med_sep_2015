...
\begin{columns}
  ...
  \block{Introduction}{\acp{EV} are small (40-1000nm) compartments
    ...
    we we trained a multiclass \ac{SVM} to accurately identify the
...
\captionof{figure}{Expression of uterus specific genes in uterus and
  tissues with the second highest expression for those genes.
  Expression is $\log_2 \mathrm{FPKM}$ and genes are tissue specific
  if they have a tissue specific expression $\tau_g \ge 0.98$. The
  known uterus specific gene PAEP and short RNAs ZNFX1-AS1\_3 (87bp)
  and AL356310.1 (94bp) are highly uterus specific.}
...
\captionof{figure}{Expression of placenta specific genes in placenta
  and tissues with the second highest expression for those genes.
  Expression is $\log_2 \mathrm{FPKM}$ and genes are tissue specific
  if they have a tissue specific expression $\tau_g \ge 0.98$ and
  $\mathrm{FPKM} \ge 1024$ in placenta. The known placenta specific
  PSG genes and CSH genes are all highly placenta specific, and short
  RNAs mir-126, mir-622, and PAPPA-AS1 are fairly placenta specific.}
}
...
\captionof{figure}{\ac{ROC} curve of reduced reads in Placenta and
  ...
  to the many fewer uterus specific genes.}
}
...
\block{Conclusions}{
  \begin{itemize}
  \item Pregnancy-specific glycoproteins (PSG1-8) are specific to
    placental tissues, and PAEP is specific to uterus, as previously
    described.
