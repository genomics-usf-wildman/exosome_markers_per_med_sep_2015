...
\begin{columns}
  ...
  \block{Introduction}{\acp{EV} are small (40-1000nm) compartments
    ...
    tissue specific genes from the other %
    %
...
%     individual patient. 
  ...
  \block{Methods}{ %
    ...
    RNAseq reads from %
    % 
    \Sexpr{nrow(genes.training)+nrow(genes.testing)} %
    % 
...
<<placenta.roc.curve,fig.width=8,fig.height=5,echo=FALSE>>=
...
\block{Conclusions}{
  ...
  \item Pregnancy-specific glycoproteins (PSG1-8) are specific to
    placental tissues,and PAEP Progestagen-Associated Endometrial
    Protein is specific to uterus, as previously described.
  ...
  \item Non-maternal variants may also be able to distinguish between
    placental reads and maternal reads in addition to the approach
    used here.
  \end{itemize}
}
