...
\captionof{figure}{Expression of uterus specific genes
  ($\tau_g \ge 0.98$) in uterus and all tissues with the second
  highest expression for any of the uterus specific genes ordered by
  their specificity to uterus. The known uterus specific gene PAEP and
  short RNAs ZNFX1-AS1\_3 (87bp) and AL356310.1 (94bp) are highly
  uterus specific.}
...
\captionof{figure}{Expression of placenta specific genes
  ($\tau_g \ge 0.98$) with high expression ($\mathrm{FPKM} \ge 1024$)
  in placenta and all tissues with the second highest expression for
  any placenta-specific gene ordered by their specificity to placenta.
  The previously known placenta-specific PSG genes and CSH genes are
  highly placenta specific and the short RNAs mir-126, mir-622, and
  PAPPA-AS1 are fairly placenta specific.}
}
